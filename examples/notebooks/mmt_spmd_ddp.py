...
def launch_mmt_job(num_nodes=3, teamspace="my-teamspace", org="my-org"):
    ...
    This approach creates a self-contained training script that includes all necessary
    monarch code rather than relying on uploading the entire project.
    """
    # Initialize a Studio
    studio = Studio(name="multi-node-ddp-training", teamspace=teamspace, org=org)
    studio.start()

    print("Creating self-contained training script...")

    # Create the training script content that includes the necessary monarch imports and code
    training_script = """
import asyncio
import os
import torch
...
# Install monarch if not available
try:
    from monarch.actor import Actor, current_rank, endpoint, proc_mesh
except ImportError:
    print("Monarch not found. Please ensure monarch is installed in the environment.")
    import sys
    sys.exit(1)
...
class MultiNodeDDPActor(Actor):
    ...
    def _rprint(self, msg):
        print(f"Node {self.node_rank}, Local Rank {self.rank}, Global Rank {self.global_rank}: {msg}")

    @endpoint
    async def setup(self):
        self._rprint("Initializing torch distributed for multi-node training")
        self._rprint(f"World size: {self.world_size}, Master: {self.master_addr}:{self.master_port}")
    ...
    @endpoint
    async def demo_basic(self, num_epochs=10, batch_size=64, input_size=128, hidden_size=512, output_size=64):
        self._rprint("Running multi-node DDP example")
...
"""
    ...
    # Create the script directly on the studio nodes using a multi-line command
    # This avoids the need to upload files by embedding the script in the command itself
    create_and_run_command = f"""
cat > multi_node_train.py << 'EOF'
{training_script}
EOF
python multi_node_train.py
"""

    # Run a Multi-machine job
    job = MMT.run(
        command=create_and_run_command.strip(),
