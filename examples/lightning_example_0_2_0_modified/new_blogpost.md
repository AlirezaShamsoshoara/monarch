...
training_procs = this_host().spawn_procs(per_host={"gpus": 8})
...
When deployed with Monarch, each compute node runs a Monarch worker bootstrap process. This bootstrap has two simple jobs. First, routing any messages passed within the Monarch ecosystem, and next spawning arbitrary process meshes of any shape. This simplification of a cluster's responsibilities allows us to encapsulate the responsibilities of a node, creating a separation between the hardware that runs computation and the user's runtime environment. As opposed to traditional methods of launching cluster scale workflows, the worker bootstrap (and thus your allocation) survives any exceptions or disconnects from the user code, meaning it can be re-used across multiple iterations. Even if the client program finishes or crashes, the compute environment can remain alive, reducing manual intervention and improving reliability for interactive and long-running workloads.
...
Reserve your cluster through Lightning MMT by simply defining the required GPUs, number of nodes, and GPUs per node. Each worker node runs a Monarch bootstrap process that keeps the node ready to accept work. After that, you attach to the workers and create a process mesh to call your training actors.
...
# Each worker node runs a bootstrap process that keeps Monarch alive
...
if __name__ == "__main__":
    # Create a mesh with 16 processes
    process_mesh = this_host().spawn_procs(per_host={"gpus": 16})
...
Created actors are spawned on a process mesh--a group of actors with access to the reserved resources. The client first enables its own transport, then attaches to the worker nodes running Monarch's bootstrap process, and creates the process mesh from the connected workers.
...
# Get worker IPs from the MMT job
